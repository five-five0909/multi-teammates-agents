...
- TUI rendering owns no durable state. Project status comes from
  ...
  `runForeground`. TTY startup renders Overview before the first menu prompt;
  non-TTY `mta` prints help and never waits for input.
...
- The install smoke packs the real whitelist, installs to an isolated prefix,
  ...
  checks both bins, npm exec/npx semantics, absence of plugin manifests and the
  plugin launcher, the explicitly packaged control-plane document, dual-host
  apply, status, hook, project MCP initialize, Doctor, and unapply.
...
- Windows and POSIX tarball smoke with excluded Python/Rust commands, npm-only
  dual-host assets, and project/package-command MCP initialize.
